...
//        System.out.println("paintLeftover: " + Clock.getBytecodeNum());
    ...
    public void refill() throws GameActionException {
        if (state != SoldierState.REFILL && state != SoldierState.ATTACK && ((state == SoldierState.DEFAULT && rc.getPaint() < 80) || rc.getPaint() <= 25) && rc.getChips() < 2000) {
            ...
            if (closestFriendPaintTower != null && (rc.getLocation().distanceSquaredTo(closestFriendPaintTower) <= refillTowerDistanceThreshold || rc.getPaint() < 5)) {
    ...
    public void buildTower() throws GameActionException {
        if (state == SoldierState.ATTACK || state == SoldierState.REFILL) return;
    ...
    public void buildSRP() throws GameActionException {
        ...
        if (state == SoldierState.DEFAULT) {
            ...
            if (rc.canSenseLocation(loc1) && !impossibleSRPLocations.contains(loc1) && !rc.senseMapInfo(loc1).isResourcePatternCenter() && loc1.x-2 >= 0 && loc1.y-2 >= 0 && loc1.x+2 < mapWidth && loc1.y+2 < mapHeight) {
                possibleSRPLocations[0] = loc1;
            }
            MapLocation loc2 = new MapLocation(loc.x - x, loc.y + 4 - y);
            if (rc.canSenseLocation(loc2) && !impossibleSRPLocations.contains(loc2) && !rc.senseMapInfo(loc2).isResourcePatternCenter() && loc2.x-2 >= 0 && loc2.y-2 >= 0 && loc2.x+2 < mapWidth && loc2.y+2 < mapHeight) {
                possibleSRPLocations[1] = loc2;
            }
            MapLocation loc3 = new MapLocation(loc.x + 4 - x, loc.y - y);
            if (rc.canSenseLocation(loc3) && !impossibleSRPLocations.contains(loc3) && !rc.senseMapInfo(loc3).isResourcePatternCenter() && loc3.x-2 >= 0 && loc3.y-2 >= 0 && loc3.x+2 < mapWidth && loc3.y+2 < mapHeight) {
                possibleSRPLocations[2] = loc3;
            }
            MapLocation loc4 = new MapLocation(loc.x + 4 - x, loc.y + 4 - y);
            if (rc.canSenseLocation(loc4) && !impossibleSRPLocations.contains(loc4) && !rc.senseMapInfo(loc4).isResourcePatternCenter() && loc4.x-2 >= 0 && loc4.y-2 >= 0 && loc4.x+2 < mapWidth && loc4.y+2 < mapHeight) {
                possibleSRPLocations[3] = loc4;
            }
            MapLocation loc5 = new MapLocation(loc.x - 4 - x, loc.y - y);
            if (rc.canSenseLocation(loc5) && !impossibleSRPLocations.contains(loc5) && !rc.senseMapInfo(loc5).isResourcePatternCenter() && loc5.x-2 >= 0 && loc5.y-2 >= 0 && loc5.x+2 < mapWidth && loc5.y+2 < mapHeight) {
                possibleSRPLocations[4] = loc5;
            }
            MapLocation loc6 = new MapLocation(loc.x - x, loc.y - 4 - y);
            if (rc.canSenseLocation(loc6) && !impossibleSRPLocations.contains(loc6) && !rc.senseMapInfo(loc6).isResourcePatternCenter() && loc6.x-2 >= 0 && loc6.y-2 >= 0 && loc6.x+2 < mapWidth && loc6.y+2 < mapHeight) {
                possibleSRPLocations[5] = loc6;
            }
            MapLocation loc7 = new MapLocation(loc.x + 4 - x, loc.y - 4 - y);
            if (rc.canSenseLocation(loc7) && !impossibleSRPLocations.contains(loc7) && !rc.senseMapInfo(loc7).isResourcePatternCenter() && loc7.x-2 >= 0 && loc7.y-2 >= 0 && loc7.x+2 < mapWidth && loc7.y+2 < mapHeight) {
                possibleSRPLocations[6] = loc7;
            }
            MapLocation loc8 = new MapLocation(loc.x - 4 - x, loc.y + 4 - y);
            if (rc.canSenseLocation(loc8) && !impossibleSRPLocations.contains(loc8) && !rc.senseMapInfo(loc8).isResourcePatternCenter() && loc8.x-2 >= 0 && loc8.y-2 >= 0 && loc8.x+2 < mapWidth && loc8.y+2 < mapHeight) {
        ...
        if (locInfo.getPaint() == PaintType.EMPTY) {
            ...
            for (MapInfo info : nearbyPaintLocations) {
                if (info.getPaint() == PaintType.EMPTY) {
                    ...
                    for (MapLocation ruin : nearbyRuins) {
                        if (info.getMapLocation().distanceSquaredTo(ruin) <= 8 && rc.getNumberTowers() < 25 && rc.senseRobotAtLocation(ruin) == null) {
                            nearRuin = true;
                            break;
                        }
                    }
