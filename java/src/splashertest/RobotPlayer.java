...
//                System.out.println("Built a tower at " + targetLoc + "!");
    ...
    public static void runSplasher(RobotController rc) throws GameActionException{
        ...
        if (curRuin != null){
            MapLocation targetLoc = curRuin.getMapLocation();
            Direction dir = rc.getLocation().directionTo(targetLoc.add(Direction.NORTH));
            if (rc.canMove(dir))
                rc.move(dir);

            RobotInfo robot = rc.senseRobotAtLocation(targetLoc);
            if (robot != null && robot.getTeam() == rc.getTeam().opponent()) {
                if (rc.canAttack(targetLoc.add(Direction.NORTH))) rc.attack(targetLoc.add(Direction.NORTH));
            }
        }
